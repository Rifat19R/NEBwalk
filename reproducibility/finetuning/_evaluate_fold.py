...
def _load_geometries(held_out: str):
    ...
    return by_index[0].copy(), by_index[3].copy(), (
...
def main(held_out: str) -> None:
    ...
    print(f"=== {SYSTEMS[held_out].symbol} held out; true DFT barrier "
    ...
    print(f"zero-shot MACE-MP-0 medium: predicted={zero_shot_barrier:.4f} eV, "
    ...
    for condition in ("al", "random"):
        ...
        for seed in SEEDS:
            ...
            print(f"  {condition} seed={seed}: predicted={predicted:.4f} eV, "
                  f"|error|={error:.4f} eV")
            per_seed.append(
